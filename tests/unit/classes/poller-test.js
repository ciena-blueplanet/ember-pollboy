...
describe('Unit : Class | Poller', function () {
  it('should poll at an interval', function (done) {
    const poller = createPoller(done)
    poller.start()
  })

  it('should be able to stop polling', function (done) {
    const spy = sinon.spy()
  ...
  it('should be able to be paused', function (done) {
    const spy = sinon.spy()
    const poller = createPoller(spy)
    poller.start()
    poller.pause()
  ...
  it('should be able to resume from being paused', function (done) {
    const spy = sinon.spy()
    const poller = createPoller(spy)
    poller.start()
    poller.pause()
